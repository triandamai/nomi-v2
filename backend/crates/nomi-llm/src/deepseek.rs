...
/// DeepSeek speaks the same chat-completions wire format as OpenAI (no `/v1` prefix on its
/// paths, unlike OpenAI's own API), with two differences this module cares about: reasoning is
/// controlled by a `thinking: { type: "enabled" | "disabled" }` object rather than a flat
/// string param, and defaults to *enabled* at high effort — so unlike OpenRouter's "always safe
/// to send" unified param, this module always sends `thinking` explicitly one way or the other,
/// to avoid silently burning reasoning tokens on calls that never opted in. Streamed deltas
/// carry reasoning text in a separate `reasoning_content` field, distinct from `content`.
...
impl DeepSeekProvider {
    ...
    fn build_body(&self, request: &LlmRequest, stream: bool) -> serde_json::Value {
        ...
        for m in &request.messages {
            ...
            for block in &m.content {
                match block {
                    ...
                    // Dropped when replaying history: DeepSeek's own reasoning_content is only
        ...
        // Always sent explicitly (not just when enabling) — DeepSeek defaults to "enabled" at
        // high effort, and one-shot utility calls (title generation, BYOK validation, ...) never
        // set enable_reasoning, so leaving this unset would silently reason on every one of them.
        body["thinking"] = json!({ "type": if request.enable_reasoning { "enabled" } else { "disabled" } });
        if stream {
...
pub async fn list_models(client: &reqwest::Client, api_key: &str, base_url: &str) -> Result<Vec<crate::ModelSummary>, LlmError> {
    ...
    if !response.status().is_success() {
        ...
        return Err(LlmError::ProviderError(format!("deepseek returned {status}: {text}")));
    }
    ...
    async fn complete_stream(&self, request: LlmRequest) -> Result<LlmEventStream, LlmError> {
        ...
        let stream = try_stream! {
            ...
            loop {
                ...
                if let Some(text) = delta.and_then(|d| d.get("reasoning_content")).and_then(|c| c.as_str()) {
