...
- **Access token**: a JWT, signed HS256 with a server-side secret read from an environment variable, 30-minute expiry. Payload:
  ```json
  {
    ...
    "permissions": ["nomi:admin:user:[view,manage]", "nomi:<org_id>:member:[view,manage]", "nomi:<org_id>:conversation:[view,manage]", "..."],
    "exp": 1234567890,
    "iat": 1234567890
  }
  ```
  **Correction from the design conversation**: `permissions` entries are *resource*-shaped (`nomi:<scope>:<resource>:[actions]`, resources `user`/`member`/`conversation`), matching the RBAC design's confirmed grammar — not role-shaped. Role names (`owner`/`admin`/`member`) never appear in a permission string; they exist only as the `memberships.role` column, used to *compute* which resource permissions a membership grants, via the RBAC design's fixed role→action table:
  - `owner` and `admin` both yield `nomi:<org_id>:member:[view,manage]` and `nomi:<org_id>:conversation:[view,manage]` — at this MVP stage the two roles are indistinguishable in the grammar, since owner's only extra powers (billing, org deletion) aren't modeled as a resource yet (see the RBAC design's Out of Scope). Anything that later needs to tell owner apart from admin queries `memberships.role` directly, the same way §4's sensitive-write re-validation already does.
  - `member` yields `nomi:<org_id>:member:[view]` and `nomi:<org_id>:conversation:[view]` — conversation *participation* (sending messages) is separately gated by `session_participants`/inferred channel membership per the RBAC design, not by this grammar, so `member`'s conversation string is view-only.
  `permissions` contains **every** resource-permission string the user currently holds — the platform-admin string if `is_platform_admin`, plus both resource strings for every `active` `memberships` row, not just the currently-active org. This makes the token self-sufficient for any read/view authorization check across every org the user belongs to, with no DB round-trip mid-request, and keeps `Claims::has_permission(scope, resource, action)` (§4) meaningful as written.
