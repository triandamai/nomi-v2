...
- **`handle_inbound_message` stays exactly as it is today**, unmodified, and every test that drives it directly (`turn_handle_inbound_message.rs`, `turn_subagent_state_machine.rs`) keeps passing unmodified. This is a deliberate refinement over treating it as a rename target: moving the inbound-message insert out from under the advisory lock (so the HTTP ingest phase can insert-and-return without waiting for a slow in-flight turn) changes the row-ordering guarantee `handle_inbound_message`'s own `concurrent_messages_for_the_same_session_are_serialized` test depends on (today, insert happens *after* lock acquisition, so a call's inbound-insert and reply-insert are atomic as a pair; splitting ingest out from processing would let two calls' inbound inserts race ahead of either's reply). Rather than accept that behavior change on a function with direct test coverage of that exact guarantee, `handle_inbound_message` keeps doing bootstrap+lock+insert+dispatch+unlock in one call, sharing only its inner routing/dispatch logic (via a `run_locked_turn` helper) with the new `process_turn` — two entry points, one shared core, not a rename.
- **Chitchat, routing, memory, and subagent/tools tests** (`turn_chitchat.rs`, `turn_money_agent.rs`, etc.) are otherwise unaffected — `turn_chitchat.rs`'s call sites gain one additional `None` argument (the new `mqtt` parameter, unused when driven directly rather than through `process_turn`), nothing else changes.
- **`process_turn` gets its own new tests** (`turn_process.rs`) covering the queue-driven path specifically: a successful reply and a `TurnFailed` event on LLM failure, both starting from an already-`ingest_inbound_message`'d row rather than re-bootstrapping.
