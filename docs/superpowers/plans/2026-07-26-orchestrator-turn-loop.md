...
- **Resolved during implementation (Task 1, fix round 1, commit `4fe427a`):** two truly concurrent *first-contact* messages from the same brand-new `(channel, channel_user_id)` (or the same brand-new `chat_id`) could otherwise race on `channel_identities`' and `sessions`' unique constraints during bootstrap (bootstrap runs before the session lock is acquired, so it isn't serialized by it). This was flagged by Task 1's reviewer and fixed with `INSERT ... ON CONFLICT DO NOTHING RETURNING id`, rolling back and re-resolving the winning row on a lost race for `channel_identities`, and an insert-first-then-select-fallback pattern for `sessions`. Covered by two dedicated concurrency tests in `turn_bootstrap.rs`.
