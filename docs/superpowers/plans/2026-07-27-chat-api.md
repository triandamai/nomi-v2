...
**Architecture:** A small new module (`backend/src/web_identity.rs`) bridges an authenticated web user to the existing channel/session bootstrap model without risking duplicate users. Four new route handlers in `backend/src/routes/sessions.rs`. `AppState`/`main.rs` gain real `LlmProvider`/`EmbeddingProvider` instances (nothing constructs one today). No changes to `handle_inbound_message`, `bootstrap_identity_and_session`, or any turn-loop/auth code — this plan is purely additive.
...
- `handle_inbound_message`, `bootstrap_identity_and_session`, and every turn-loop/auth file from prior plans are untouched by this plan except the two existing test files (`turn_handle_inbound_message.rs`-adjacent auth test files — see Task 1) that construct `AppState` directly and must be updated to supply the two new fields.
...
**Interfaces:**
- Consumes: `web_identity::ensure_web_channel_identity` (Task 2), `bootstrap::bootstrap_identity_and_session` (existing, untouched), `auth::authorize::authorize_org_action` (existing, untouched).
